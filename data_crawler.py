...
class DataCrawler:
    ...
    def get_links(content):
        result = []
        for string in list(map(str, soup.find_all('td'))):
            if "href" in string:
        ...
        for val in result:
            ...
            while val[index] != "\"":
                href += val[index]
                index += 1
            href_lst.append("https://markets.businessinsider.com/bond/historical/" + href)
            names.append(href[-12:])
        return dict(zip(href_lst, names))
...
if __name__ == '__main__':
    ...
    links = DataCrawler.get_links(soup)
